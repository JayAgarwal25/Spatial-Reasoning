...
def draw_scene_graph(scene_graph: SceneGraph, output_path: str) -> None:
    image = cv2.imread(step1_scene_graph.image_path)
    if image is None:
        raise ValueError(f'Could not read image: {step1_scene_graph.image_path}')
    for node in step1_scene_graph.nodes:
    ...
    node_map = {n.id: n for n in step1_scene_graph.nodes}
    for edge in step1_scene_graph.edges:
